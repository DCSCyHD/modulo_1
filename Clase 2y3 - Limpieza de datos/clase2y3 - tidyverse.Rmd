...
##~~~~~~~~~~~~~~~~~~~x~~~~~~~~~~~~~~~
eph_3t2023 <- read.table("bases/EPH_usu_3er_Trim_2022_txt/usu_individual_T322.txt.txt", 
                             header = TRUE, sep = ";", dec = ",")
```
...
```r
eph_3t2023 <- read.delim("bases/EPH_usu_3er_Trim_2022_txt/usu_individual_T322.txt.txt", 
                             header = TRUE, 
                             sep = ";", 
                             dec = ",")
```
...
```{r eval=F}
### Calculo la media
round(prop.table(table(`eph_3t2023$ESTADO`)), digits = 2)
```
...
```{r eval=F}
base_ejercicio <- select(eph_3t2023, ESTADO, CH04, CAT_OCUP)
```
