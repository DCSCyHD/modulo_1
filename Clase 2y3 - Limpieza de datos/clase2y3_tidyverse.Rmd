...
`r chunk_reveal("select_1", widths = c(1.1, 1.9), title = "# Otra forma de seleccionar")`
...
`r chunk_reveal("select_2", widths = c(1.3, 1.7), title = "# Otra forma de seleccionar")`
...
`r chunk_reveal("select_3", widths = c(1.3, 1.7), title = "# Otra forma de seleccionar")`
...
`r chunk_reveal("select_4", widths = c(1.3, 1.7), title = "# Otra forma de seleccionar")`
...
`r chunk_reveal("select_5", widths = c(1.3, 1.7), title = "# Otra forma de seleccionar")`
...
- **Indicador2:** Quiero conocer el mínimo y máximo valor del ingreso de la ocupación principal
...
eph_3t2022 %>% #BREAK
  select(AGLOMERADO, CH04, CH06, ESTADO, PONDERA,P21) %>% #BREAK
  mutate(edad_rango = case_when(CH06 %in% c(0:18) ~  "0 a 18",
                                CH06 %in% c(19:29) ~ "19 a 29",
                                CH06 %in% c(30:39) ~ "30 a 39",
                                CH06 %in% c(40:49) ~ "40 a 49",
...
eph_3t2022 %>%
  ...
  summarise(cant_pob_tot = sum(PONDERA),
            ...
            min_ingr_oc_princ = min(P21),
            max_ingr_oc_princ = max(P21)) %>% 
  select(CH04, cant_ocupados, cant_pob_tot) %>% 
  pivot_longer(cols = c(cant_ocupados, cant_pob_tot), #<<
               names_to = "variable", 
               values_to = "valor")
```
